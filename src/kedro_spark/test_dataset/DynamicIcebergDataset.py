...
class DynamicIcebergDataset(SparkDataset):
    ...
    def _data_validation(self) -> None:
        ...
        table_name, missing_dates = check_data_completeness_auto(
            path=self._full_spark_table_name ,
        ...
        if not missing_dates:
            logger.info(
                f"All expected data is available for table '{table_name}' at path: {self._full_spark_table_name}"
            )
        else:
            error_message = (
                f"Missing partitions for table '{table_name}' at path: {self._full_spark_table_name}. "
